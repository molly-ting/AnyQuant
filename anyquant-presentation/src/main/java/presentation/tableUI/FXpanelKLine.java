...
public class FXpanelKLine extends JPanel {

	private JFXPanel fxpanel;
	private int width, height;
	private int num;
	private double dataMax, dataMin;
	private double dataGap, klinemin, klinemax, amin, amax, min, max;
	private NumberAxis yAxis;

	public FXpanelKLine(ArrayList<ShareVO> dataList, ArrayList<AverageVO> avgdataList, int width, int height) {
		this.width = width;
		this.height = height;
		...
		if (dataList != null && avgdataList != null) {
			this.add(fxpanel);
			initchart(dataList, avgdataList);
		} else {
			initnotfound();
		}
	}

	public void setdatalist(ArrayList<ShareVO> dataList, ArrayList<AverageVO> avgdataList) {
		this.removeAll();
		if (dataList != null && avgdataList != null) {
			this.add(fxpanel);
			initchart(dataList, avgdataList);
	...
	private void initchart(ArrayList<ShareVO> dataList, ArrayList<AverageVO> avgdataList) {
		Platform.runLater(new Runnable() {
			...
			public void run() {
				...
				CandleStickChart chart = createChart(dataList, avgdataList);
	...
	protected CandleStickChart createChart(ArrayList<ShareVO> dataList, ArrayList<AverageVO> avgdataList) {
		// X�ᣬ��Χ0-32�����1
		get_kline_max_and_min(dataList);
		get_average_max_and_min(avgdataList);
		min = Math.min(amin, klinemin);
		max = Math.max(amax, klinemax);
		dataGap = (double) (max - min) / 20.0;
		dataMin = Math.max(min - 0.3, 0);
		...
		int size = Math.min(dataList.size(), avgdataList.size());
		num = size;
		int count = 1;
		for (int i = dataList.size() - size; i < dataList.size(); i++, count++) {
			...
			series.getData().add(
					...
					new XYChart.Data<String, Number>(temp.getDate(), temp.getOpen(),
							new CandleStickExtraValues(temp.getDate(), temp.getClose(), temp.getHigh(), temp.getLow(),
									avgdataList.get(avgdataList.size() - count).getValue())));
		}
			...
			for (int seriesIndex = 0; seriesIndex < getData().size(); seriesIndex++) {
				...
				while (iter.hasNext()) {
					...
					if (itemNode instanceof Candle && extra != null) {
						...
						candle.updateTooltip(extra.getDate(), item.getYValue().doubleValue(), extra.getClose(),
	...
	private class CandleStickExtraValues {
		...
		public CandleStickExtraValues(String date, double close, double high, double low, double average) {
			this.date = date;
			this.close = close;
			this.high = high;
			this.low = low;
	...
	private class Candle extends Group {
		...
		public void updateTooltip(String date, double open, double close, double high, double low) {
			TooltipContent tooltipContent = (TooltipContent) tooltip.getGraphic();
			tooltipContent.update(date, open, close, high, low);
	...
	private class TooltipContent extends GridPane {
		...
		private TooltipContent() {
			...
			setConstraints(open, 0, 1); // ������Node column row
		...
		public void update(String date, double open, double close, double high, double low) {

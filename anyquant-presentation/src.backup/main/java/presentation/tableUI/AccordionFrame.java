...
public class AccordionFrame extends JPanel {
	...
	private void init() {
		...
		String[] labelnames = { "Âº?Áõò‰ª∑ÔºàÂÖÉÔº?", "Êî∂Áõò‰ª∑ÔºàÂÖÉÔºâ", "Êú?È´ò‰ª∑ÔºàÂÖÉÔº?", "Êú?‰Ωé‰ª∑ÔºàÂÖÉÔº?", "Êàê‰∫§ÈáèÔºàËÇ°Ôºâ" };
		...
		for (int i = 1; i < 6; i++) {
			...
			tips[i][0] = new JLabel("*Âè™ËÉΩÂ°´Êï∞Â≠?");
			...
			tips[i][1] = new JLabel("*Âè™ËÉΩÂ°´Êï∞Â≠?");
	...
	private void filter() {
		...
		info += "Êó•ÊúüÔº?" + start + "‚Ä?" + end;
		ArrayList<ShareVO> result = filterbl.filter(strategy, shareinit);
		String[] head = { "", "Âº?Áõò‰ª∑", "Êî∂Áõò‰ª?", "Êú?È´ò‰ª∑", "Êú?‰Ωé‰ª∑", "Êàê‰∫§Èá?" };
		...
		for (int i = 1; i < 6; i++) {
			...
			if (starttemp.equals("Êú?Â∞èÂ??") || starttemp.equals("")) {
				startnull = true;
			...
			if (endtemp.equals("Êú?Â§ßÂ??") || endtemp.equals("")) {
				endnull = true;
			...
			if ((!startnull) && endnull) {
				info += " > " + head[i] + "Ôº?>" + starttemp;
			} else if (startnull && (!endnull)) {
				info += " > " + head[i] + "Ôº?<" + endtemp;
			} else if ((!startnull) && (!endnull)) {
				info += " > " + head[i] + "Ôº?" + starttemp + "‚Ä?" + endtemp;
			}
	...
	private void clear() {
		...
		for (int i = 1; i < 6; i++) {
			for (int j = 0; j < 2; j++) {
				if (j == 0) {
					fields[i][j].setText("Êú?Â∞èÂ??");
					fields[i][j].addMouseListener(minListener);
				} else {
					fields[i][j].setText("Êú?Â§ßÂ??");
			...
			for (int i = 1; i < 6; i++) {
				for (int j = 0; j < 2; j++) {
					if (e.getSource() == fields[i][j]) {
						...
						if (text.equals("Êú?Â§ßÂ??") || text.equals("Êú?Â∞èÂ??")) {
							fields[i][j].setText("");
						}
		...
		public void focusLost(FocusEvent e) {
			...
			for (int i = 1; i < 6; i++) {
				for (int j = 0; j < 2; j++) {
					if (e.getSource() == fields[i][j]) {
						String text = fields[i][j].getText();
						if (!(text.equals("") || text.equals("Êú?Â∞èÂ??") || text.equals("Êú?Â§ßÂ??"))) {
			...
			if (e.getSource() == confirm) {
				...
				for (int i = 1; i < fields.length; i++) {
					for (int j = 0; j < fields[i].length; j++) {
						String text = fields[i][j].getText();
						if (!(text.equals("Êú?Â∞èÂ??") || text.equals("Êú?Â§ßÂ??") || text.equals(""))) {
							if (!check.checkNumber(fields[i][j].getText())) {
								tips[i][j].setVisible(true);
								correct = false;
							} else {

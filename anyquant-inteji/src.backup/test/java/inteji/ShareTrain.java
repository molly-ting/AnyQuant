...
public class ShareTrain {
	/**
	 * 学习�?
	 */
	private double eta = 0.000000001;
	/**
	 * 目标误差
	 */
	...
	public void train(String id) {
		...
		ArrayList<ShareDTO> list = db.getData(id, "2015-02-01", "2016-05-10");

		IntejiTool inte = new IntejiTool();
		double[] x = inte.getData(id, list.get(list.size() - 1).getDate(), list.size() + input);
		...
		while (true) {
			...
			while (iteration-- > 0) {
				...
				if (calTotalError(x) < ERR) // 误差已达到要求，可以�?出迭�?
			...
			ArrayList<ShareDTO> test = db.getData(id, "2016-05-01", "2016-05-20");
			...
			for (int i = 0; i < test.size() - 1; i++) {
				double a = getOutput(test.size() - i - 1, t);
				double b = test.get(i + 1).getClose();
				System.out.println(a + " " + b);
				result += Math.abs(a - b);

			}
			System.out.println("总偏�?:" + result);

			if (result > previous || result <= 5) {
	...
	private void initValue(String id, double[] x) {
